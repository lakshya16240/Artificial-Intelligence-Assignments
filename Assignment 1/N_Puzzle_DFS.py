...
def process(a):
    ...
    if i == 0 and j == 0:
        ...
        if not test:
            process_space.append(Node(b, i + 1, j, name, a.steps + 1))
        ...
        test = isExisting(name)
        if not test:
            process_space.append(Node(b, i , j+1, name, a.steps + 1))
...
count = 0
process_space.append(matrix)
boolean = False
while len(process_space) > 0:
    node = process_space.pop()
    processed.add(node.name)
    count = count + 1
    boolean = process(node)
    if boolean :
        break
    # print("length of stack " + str(len(process_space)))
if not boolean:
